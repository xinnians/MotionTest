...
public class MProgressBar extends View {
    ...
    private ButtonProgressBar.Type mLoaderType = ButtonProgressBar.Type.DETERMINATE;
    ...
    public void init(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            switch (type) {
                case 0:
                    mLoaderType = ButtonProgressBar.Type.DETERMINATE;
                    break;
                case 1:
                    mLoaderType = ButtonProgressBar.Type.INDETERMINATE;
                    break;
            }
        } finally {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int widthSpec = MeasureSpec.makeMeasureSpec (400, MeasureSpec.UNSPECIFIED);
        int heightSpec = MeasureSpec.makeMeasureSpec (400, MeasureSpec.UNSPECIFIED);
        progressBar.measure(widthSpec, heightSpec);
     ...
     * @param mLoaderType
     */
    public void setLoaderType(ButtonProgressBar.Type mLoaderType) {
        this.mLoaderType = mLoaderType;
    }

    public ButtonProgressBar.Type getLoaderType() {
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if (mLoaderType == ButtonProgressBar.Type.DETERMINATE) {
    ...
    public void onDrawProgress(Canvas canvas) {
        ...
        Rect bounds = new Rect();
        mTextPaint.getTextBounds(mLoadingText, topX, mLoadingText.length(), bounds);
        int xPos = (canvas.getWidth() - bounds.width())/ 2;
        int textAdjust = (int) (mTextPaint.descent() + mTextPaint.ascent()) / 2;
        int yPos = ((canvas.getHeight() / 2) - textAdjust);
        canvas.drawText(mLoadingText, xPos, yPos, mTextPaint);
    }
     ...
     */
    public void reset() {
        if (mLoaderType == ButtonProgressBar.Type.INDETERMINATE) {
